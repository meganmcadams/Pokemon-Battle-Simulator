...
def battle_simulator(trainers, items, pokemon, stored_pokemon, parties, moves, to_next_level, move_levels):
    ...
    party1, party2, party1_pokemon, party2_pokemon = validate_parties(stored_pokemon, parties, party1,
                                                                      party2)  # validate the parties
    ...
    health_check(party_order, party1_pokemon, party2_pokemon, party_order_names,
                 fainted_pokemon)  # make sure no pokemon start fainted
    ...
    while True:
        ...
        for curr_pokemon in party_order:  # go through pokemon's turns one by one
            ...
            if int(curr_pokemon['Curr HP']) > 0:  # if the pokemon can do something and isn't dead
                ...
                subheader(current_party['Name'] + " | " + party_order_names[curr_pokemon['ID']] + " | " + str(
                    "Cycle " + str(cycle) + ", Turn " + str(turn)))
                ...
                if curr_move != -2:  # if not pass
                    ...
                    if curr_move == -1:  # throw pokeball
                        throw_pokeball(pokemon, items, trainers, current_party, opposing_party, target,
                                       curr_pokemon['Tid'])
            ...
            curr_health_check = health_check(party_order, party1_pokemon, party2_pokemon, party_order_names,
                                             fainted_pokemon)
